...
up360.Rendering.Engine = function(parentElement, settings) {
    ...
    var currentLevelObject, highestLevelObject, currentFrame, sourceRectangle, imageRepository, startFramesLoader, zoomValue, minZoomValue, _scale, posX = 0, posY = 0, mousePosX = 0, mousePosY = 0, isMouseOnScreen = false, screenRectangle, contentWidth, contentHeight, InterfaceObject, _this = this, moveRedrawTimeout;
...
up360.init = function(element, options) {
    var parentContainer, rootElement, settings, engine, lastNormalizedZoomValue = 0;
    if (element instanceof HTMLElement) parentContainer = element; else if (typeof element === "string") parentContainer = document.querySelector(element); else throw "up360: Invalid container element provided.";
    ...
    settings.onInitComplete.Add(function() {
        if (settings.DOMSettings.buildUI) ui.Init();
        if (settings.DOMSettings.buildPreviewWindow) previewWindow.Init();
        inputManager.Init();
        preLoader.Hide();
        engine.UpdateSize();
    ...
    ui.OnPlayChanged.Add(function(command) {
        if (command === "play") animation.Play(); else animation.Stop();
    });
    ...
    var interfaceObject = {
        Init: function() {
            if (settings.DOMSettings.buildLoadingScreen) preLoader.Init();
            preLoader.Show();
            engine.Init();
        },
        Zoom: engine.Zoom,
