...
immediate and lazy 
*/
export const StartFramesLoader = function(imageRepository, settings, completeCallback){
	...
	*/
	this.immediateLoader = function() {
		...
		for (var i = settings.minFrame; i <= settings.maxFrame; i++) {
			imageRepository.getImage(i, 0, 0, settings.levelObjects[0], settings.resourceUrl, loadCallback);
		}
	};
